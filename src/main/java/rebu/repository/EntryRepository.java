...
public interface EntryRepository extends JpaRepository<Entry,Long> {
    ...
    @Query("select distinct entry from Entry entry left join fetch entry.titles")
    List<Entry> findAllWithEagerRelationships();

    @Query("select entry from Entry entry left join fetch entry.titles where entry.id =:id")
